...
def fixedPoint(n,x_0, y_0, z_0):
    x_i = lambda a,b: (13-a-2*b)/5
    y_i = lambda a,b: (4+b-a)/3
    z_i = lambda a,b: (17-a-2*b)/4
    for i in range(n):
        x = x_i(y_0,z_0)
        y = y_i(x_0, z_0)
        z = z_i(x_0, y_0)
...
#    real = [0.5,0.5,1/3]
#    x = np.matrix('0;0;0')
#    A = np.matrix('10 12 15;6,8,12;12,12,18')
#    b = np.matrix('16;11;18')
#    D = np.diag(np.diag(A))
#    L = np.tril(A)
#    U = np.triu(A)-D
#    DLinverse = np.linalg.inv(L)
